...
import mysql.connector  # ✅ Usamos solo este
from flask import (
    Flask,
    render_template,
    request,
...
app.config["MAIL_USERNAME"] = "oscarmalaga4@gmail.com"
app.config["MAIL_PASSWORD"] = "vhdorsnxvmgdfbqv"
app.config["MAIL_DEFAULT_SENDER"] = "oscarmalaga4@gmail.com"
...
def confirm_email(token):
    try:
        ...
        # Realizar la consulta para actualizar el estado del usuario
        cur.execute("UPDATE users SET email_confirmed = 1 WHERE confirmation_token = %s", (token,))
        
        # Verificar si se afectaron filas (es decir, si el token era válido)
        if cur.rowcount == 0:
            # Si no se afectaron filas, significa que el token no existe en la base de datos
            return render_template("confirmation_failure.html", message="Token inválido o ya confirmado.")
        ...
        # Si la confirmación fue exitosa, mostrar la página de éxito
        return render_template("confirmation_success.html")
    
    except Exception as e:
        # Manejo de errores generales
        print(f"Error al confirmar el usuario: {e}")
        return render_template("confirmation_failure.html", message="Ocurrió un error al procesar la confirmación.")
...
def save_user_with_token(username, email, hashed_password, token, fecha_nacimiento, genero, curp):
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    try:
        cur.execute(
            """
            INSERT INTO users (
                username, email, password, id_rol, confirmation_token, fecha_nacimiento, genero, curp
            ...
            (
                ...
                email,
                hashed_password,
                2,  # Asignar id_rol con valor 2 por defecto
                token,
                fecha_nacimiento,
                genero,
                curp,
        ...
        conn.commit()  # ✅ AQUÍ el commit correcto
